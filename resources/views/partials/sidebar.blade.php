    ...
    <div class="kiu-sidebar-card">
        ...
        @if(auth()->user()->isAdmin())
            <nav class="d-flex flex-column gap-1">
                ...
                <a class="kiu-side-link {{ request()->routeIs('applications.*', 'professor-applications.*') ? 'active' : '' }}" href="{{ route('applications.index') }}"><i class="bi bi-inbox"></i> Applications</a>
                <a class="kiu-side-link {{ request()->routeIs('tasks.*') ? 'active' : '' }}" href="{{ route('tasks.index') }}"><i class="bi bi-list-check"></i> Tasks</a>
            </nav>
        @elseif(auth()->user()->isStudent())
            <nav class="d-flex flex-column gap-1">
                <a class="kiu-side-link {{ request()->routeIs('student.vacancies.*', 'student.feed', 'student.apply*') ? 'active' : '' }}" href="{{ route('student.vacancies.index') }}"><i class="bi bi-briefcase"></i> Opportunities</a>
        ...
        @elseif(auth()->user()->isProfessor())
            <nav class="d-flex flex-column gap-1">
                <a class="kiu-side-link {{ request()->routeIs('professor.portal', 'professor.vacancies.*', 'professor.apply*') ? 'active' : '' }}" href="{{ route('professor.portal') }}"><i class="bi bi-briefcase"></i> Opportunities</a>
